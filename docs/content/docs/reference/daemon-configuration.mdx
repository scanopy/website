...
Specifies a credential ID to seed during daemon registration. Can be repeated to seed multiple credentials. Credentials specified here will be used during the daemon's first scan and auto-assigned to discovered hosts at matching IPs. See [Credentials](/using-scanopy/credentials/#during-daemon-setup-new-daemons) for details.
