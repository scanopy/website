...
Specifies a credential ID to seed during daemon registration. Can be repeated to seed multiple credentials. Credentials specified here will be used during the daemon's first scan and [auto-assigned](/using-scanopy/credentials/#auto-assignment) to discovered hosts at matching IPs. See [Credentials](/using-scanopy/credentials/#during-daemon-setup-new-daemons) for details.
