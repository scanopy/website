...
**Docker container not exposed** — Containers on internal bridge networks aren't reachable via network scanning. Enable Docker container discovery (local socket or [Docker Proxy](/guides/docker-proxy/)), which inspects the Docker API directly.
