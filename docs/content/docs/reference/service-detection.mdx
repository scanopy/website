...
**Docker container not exposed** — Containers on internal bridge networks aren't reachable via network scanning. Enable Docker container discovery (local socket or [Docker Proxy](/guides/integrations/docker/)), which inspects the Docker API directly.
...
**Service on a raw socket port (9100-9107)** — Ports 9100-9107 are skipped by default to prevent ghost printing on JetDirect printers, since these ports interpret any TCP data as print input. This affects Prometheus Node Exporter (port 9100) and JetDirect printers. Enable **Probe Raw Socket Ports** in your discovery's scan settings to include these ports.
