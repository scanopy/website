...
**Docker Compose** — the published `docker-compose.yml` reads a `.env` file placed beside it and passes everything in it to the server, so most settings need no change to the compose file. The file is optional; Compose starts without one. To set a variable in the compose file itself, add it to the `server` service:
...
| **Web External Path**     | N/A                            | `SCANOPY_WEB_EXTERNAL_PATH`          | _Embedded UI_             | Serve the web UI from this directory instead of the copy compiled into the binary. Set to `/app/static` in the Docker image; unset elsewhere unless you are replacing the bundled UI. If the directory has no readable `index.html`, the server logs a warning and serves the compiled-in UI |
...
**Reference in docker-compose.yml**: the published compose file already does this, so a `.env` beside it is picked up as it is. In a compose file of your own:
