...
Which credential a host is queried with follows the standard [resolution order](/using-scanopy/credentials/#credential-resolution). SNMP adds one step of its own: the community string `public` is tried as a last resort, so a device left on the default responds without any credential configured.

If every credential fails on both UDP 161 and 1161, the host is discovered without SNMP-sourced data. Individual query failures within a successful session are logged and skipped — they do not abort the rest of the collection.
