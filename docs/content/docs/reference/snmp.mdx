...
Technical reference for Scanopy's SNMP implementation. For setup instructions, see the [SNMP discovery guide](/guides/integrations/snmp/). For how credential assignment and resolution works, see [Credentials](/using-scanopy/credentials/).
...
| --- | --- | ---------- | ---------------- |
| **System MIB** | RFC 3418 | `1.3.6.1.2.1.1` | sysDescr, sysObjectID, sysUpTime, sysContact, sysName, sysLocation, sysServices |
| **IF-MIB** | RFC 2863 | `1.3.6.1.2.1.2`, `1.3.6.1.2.1.31.1.1` | ifNumber, ifDescr, ifType, ifSpeed, ifPhysAddress, ifAdminStatus, ifOperStatus, ifName, ifAlias, ifHighSpeed, ifMtu |
| **IP-MIB** | RFC 4293 | `1.3.6.1.2.1.4` | ipAdEntAddr, ipAdEntIfIndex, ipAdEntNetMask, ipNetToMediaPhysAddress, ipNetToMediaNetAddress |
| **LLDP-MIB** | IEEE 802.1AB | `1.0.8802.1.1.2` | lldpRemSysName, lldpRemPortId, lldpRemChassisId, lldpRemSysDesc, lldpLocChassisId, lldpLocPortId, lldpLocPortDesc |
| **CDP-MIB** | Cisco proprietary | `1.3.6.1.4.1.9.9.23` | cdpCacheDeviceId, cdpCachePlatform, cdpCacheDevicePort, cdpCacheAddress |
| **ENTITY-MIB** | RFC 6933 | `1.3.6.1.2.1.47` | entPhysicalMfgName, entPhysicalModelName, entPhysicalSerialNum, entPhysicalFirmwareRev (firmware revision), entPhysicalSoftwareRev (software revision — a separate value on devices that report both, such as a bootloader and an OS version) |
| **BRIDGE-MIB / Q-BRIDGE-MIB** | RFC 4188, RFC 4363 | `1.3.6.1.2.1.17` | dot1dBaseNumPorts, dot1dTpFdbAddress (MAC learning), dot1qVlanStaticName (VLAN names), dot1qPvid (per-port native VLAN), dot1qVlanCurrentEgressPorts, dot1qVlanCurrentUntaggedPorts (VLAN membership) |
...
| ------ | -------------------------- |
| **Host** | hostname (from sysName when DNS lacks one), sysDescr, sysObjectID, sysLocation, sysContact, sysName, manufacturer, model, serial number, chassis ID, firmware revision, software revision |
...
Which credential a host is queried with follows the standard [resolution order](/using-scanopy/credentials/#credential-resolution), with one SNMP-specific addition: `public` is tried as a last resort, so a device left on the default needs no credential.
