...
Raise the limit for large networks that need a longer window. A run hitting it unexpectedly means the scan is slower than it should be — see [discovery finished with hosts unscanned](/setting-up-daemons/troubleshooting-scans/scan-performance/#discovery-finished-with-hosts-unscanned).
