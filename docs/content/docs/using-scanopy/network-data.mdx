---
title: Hosts & Subnets
description: Managing discovered hosts and network segments.
---
...
These relationships appear in the topology and host details, showing your virtualization hierarchy. The [Workloads perspective](/using-scanopy/topology/workloads) provides a dedicated view of this hierarchy.
...
Hosts discovery hasn't seen within their network's window carry a <StaleTag /> badge, and the host list has a **Stale only** filter to isolate them. This flags devices that may have gone offline without deleting them. See [Staleness](/using-scanopy/staleness/) for how the window works and why an entity goes stale.
...
Like hosts and services, subnets can go [stale](/using-scanopy/staleness/) — carrying a <StaleTag /> badge and a **Stale only** filter when discovery hasn't seen them within their network's window.
