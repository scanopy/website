...
### Locked assignments in the wizard

In the daemon setup wizard and the discovery **Credentials** tab, a credential that's already assigned to a host through that host's own **Credentials** tab appears as a **locked target row**, under an "Already assigned to:" list. It has no remove button — the assignment lives on the host, not on the discovery, so removing it here would do nothing. Give the credential an additional target and it becomes an ordinary editable row.
