...
- **Not every device publishes LLDP over SNMP** — some managed switches collect neighbor data internally without exposing the LLDP MIB, so a walk returns nothing however valid the credential is. Where the vendor's controller holds that data, [controller discovery](#controller-discovery) is the way to get it
...
**Which daemon runs it.** The daemon that last discovered the host — evidence it can reach the address, rather than a guess from subnet membership. That daemon must still exist, still be on the host's network, and still have an interface on a subnet holding one of the host's addresses.

**Where it shows up.** In the Scans tab as an AdHoc run, with the same progress and cancel controls as any other scan. Edit, Run and Toggle are suppressed on it, since it isn't a configuration you can re-run. A `Queued` phase means the daemon is busy and will start when its current scan finishes.

**How thorough it is.** Where the daemon's interface has a MAC address, it ARPs the target — which sees a live host even when every port is firewalled. On an interface without one, such as a point-to-point VPN tunnel, it falls back to a TCP probe, which cannot tell a firewalled host from an absent one. Loopback addresses aren't scannable and are excluded.
...
A rescan is recorded as an AdHoc run.
