...
<Callout type="info">
Creating a credential alone doesn't activate SNMP collection — you must assign it to a network or to specific hosts. Which credential a given host ends up using follows the usual [resolution order](/using-scanopy/credentials/#credential-resolution), with the community string `public` as the final fallback. See the [SNMP discovery guide](/guides/integrations/snmp/) for setup steps.
</Callout>
