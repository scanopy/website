...
3. Discovers Docker containers via the local socket or a [Docker Proxy credential](/guides/docker-proxy/) (local or remote)
...
- **Docker Proxy credential** — the daemon connects to a Docker socket proxy, which can be local or remote. See [Set up a Docker Proxy](/guides/docker-proxy/) for configuration.
