...
4. Select daemon mode:
   - **DaemonPoll**: All connections strictly Daemon -> Server. No firewall rules needed to allow server to reach daemon.
   - **ServerPoll**: All connections strictly Server -> Daemon. Requires a URL where the server can reach the daemon and firewall configured to allow connections from server to daemon.
