...
6. [Assign it to the target host](/guides/docker-proxy/#assigning-to-a-host) from the host edit modal (**Credentials** tab)
...
Docker and Podman container discovery run automatically when the daemon can reach a local socket — there's no mode to select. To scan a remote or access-restricted runtime, add a proxy credential instead. See [Set up a Docker Proxy](/guides/docker-proxy/) or [Set up Podman discovery](/guides/podman/).
