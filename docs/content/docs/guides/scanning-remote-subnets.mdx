...
A daemon automatically discovers every subnet its host has a network interface on and adds them to its Network Scan discovery. These **interfaced subnets** get fast Layer 2 (ARP) scanning. You can see a daemon's interfaced subnets on its card in **Discover > Daemons**.
...
To stop scanning a subnet, remove it from the Network Scan discovery's subnet list in **Discover > Scheduled**. The daemon will skip it on the next run.
