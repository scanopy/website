...
4. Enter the proxy port (default: 2375, or 2376 for TLS), optional path, and SSL certificates if applicable
5. Save the credential
6. [Assign it to the target host](/using-scanopy/credentials/#assigning-a-credential-to-a-host) from the host edit modal (**Credentials** tab)
...
See the [Docker discovery guide](/guides/integrations/docker/) for the full setup process.
...
When creating a new daemon or editing an existing discovery, a **Credentials** tab lets you add SNMP and Docker Proxy credentials with target IPs. Credentials are [auto-assigned](/using-scanopy/credentials/#auto-assignment) to discovered hosts after the first scan. See [Credentials](/using-scanopy/credentials/) for details.

### Container discovery

Docker and Podman container discovery run automatically when the daemon can reach a local socket — there's no mode to select. To scan a remote or access-restricted runtime, add a proxy credential instead. See [Set up Docker discovery](/guides/integrations/docker/) or [Set up Podman discovery](/guides/integrations/podman/).
