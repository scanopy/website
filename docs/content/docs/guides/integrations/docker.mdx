...
Discovery is local to one host: the daemon sees the containers on its own machine, or on the single host a proxy credential points at. See [Docker host matching](/using-scanopy/discovery/#container-host-matching) for what that means for deduplication.
