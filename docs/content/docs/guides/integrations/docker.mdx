...
The daemon scans the local socket automatically whenever it's accessible — there's no toggle to switch on. Create a **Docker Socket** credential only when you need to point the daemon at a non-default socket path.
...
4. Check that cert and key files aren't swapped — see [Credential troubleshooting](/setting-up-daemons/troubleshooting-scans/credentials/) for diagnosing file read and PEM errors in daemon logs
