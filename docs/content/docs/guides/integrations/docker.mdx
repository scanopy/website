...
Discovery is local to one host: the daemon sees the containers on its own machine, or on the single host a proxy credential points at. See [Docker host matching](/using-scanopy/discovery/#docker-host-matching) for what that means for deduplication.
...
<Callout type="warn">
**Migrating from daemon config?** If you previously configured a proxy via `--docker-proxy`, `SCANOPY_DOCKER_PROXY`, or the config file, recreate it as a Docker Proxy credential. The daemon config flags were deprecated in v0.15.0 and removed in v0.16.0. See the [migration guide](/guides/operations/unified-discovery-migration/).
</Callout>
