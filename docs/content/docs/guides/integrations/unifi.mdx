...
| Connected clients | Hosts, matched against existing hosts by IP and MAC |
...
A client is only created as a host when its address falls inside a subnet Scanopy knows, so clients on subnets no daemon scans do not appear. If the controller does not return its client list, its adopted devices are still discovered.
