...
For daemon-related issues, see [Daemon Troubleshooting](/setting-up-daemons/troubleshooting-setup/).
...
**Solution**: Replace the binary with the release build, as in [Upgrading a native install](/self-hosted-server/upgrading/#upgrading-a-native-install). It serves the UI with no extra configuration. To build from source instead, build the UI first, then compile the server with `cargo build --release --bin server --features embed-ui`.
...
1. **Fill in every SMTP setting**: relay, username, password, and sender email must all be set together. With any of them missing, email stays disabled. See [SMTP Configuration](/reference/server-configuration/#smtp-configuration).
...
See [Server Configuration](/reference/server-configuration/#session-security) for details.
