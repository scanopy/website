...
This stops and deregisters the service and deletes its `config.json`. Add `--purge` to also delete the installed binary and log files, and `--name <name>` to target a specific instance. On Windows, run it from an elevated PowerShell.
