...
2. **Credentials** — optionally add SNMP and Docker Proxy credentials with target IPs. These will be [auto-assigned](/using-scanopy/credentials/#auto-assignment) to discovered hosts after the first scan. See [Credentials](/using-scanopy/credentials/#during-daemon-setup-new-daemons) for details.
