...
Each daemon also reports which subnets it has network interfaces on, which is where its default scan targets come from. That list refreshes on the daemon's next discovery, so re-run one after changing the host's network configuration.
