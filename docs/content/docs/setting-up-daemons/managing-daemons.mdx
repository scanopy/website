...
3. **Install** — a ready-to-copy install command with the correct API key, server URL, daemon name, and any `--credential-id` flags
...
<Callout type="info">
If you run multiple daemons on one host, see [Running Multiple Daemons](/guides/networking/multiple-daemons/) for the systemd template unit.
</Callout>
