...
**Symptoms**: A run completes with a [warning](/reference/scan-warnings/) like `Scan hit its time limit (6h) — 120 host(s) not scanned. Raise Max Discovery Duration or rescan.`, and those hosts show stale data.
