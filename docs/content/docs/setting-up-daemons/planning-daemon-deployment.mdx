...
- **Remote subnets**: The daemon can scan subnets it can route to but doesn't have an interface on (Layer 3 only). See [Managing Scan Targets](/guides/scanning-remote-subnets/).
- **VPN networks**: If the daemon's host is connected to a VPN, it can scan that network with full Layer 2 access. See [Scanning VPN Networks](/guides/scanning-vpn-networks/).
...
<Callout type="info">
  Both `--bind-address` and `--daemon-port` are required when running multiple daemons on one host. Without them, the second instance will fail with a port binding error. See [Configuring ports and bind addresses](/guides/multiple-daemons/#configuring-ports-and-bind-addresses) for details.
</Callout>
...
This gives you Layer 2 discovery on each segment without needing a separate physical host per VLAN. See [Scanning Isolated Networks from One Host](/guides/multiple-daemons/) for the full setup guide, including config namespacing, systemd templates, and Docker configuration.
