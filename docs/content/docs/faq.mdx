...
Not currently. See [Limitations](/limitations/) for details on what's planned.
...
See [Discovery Duration](/discovery/#discovery-duration) for benchmarks and factors affecting scan speed.
...
Yes, but you'll miss some data. See [Layer 2 vs Layer 3](/limitations/#layer-2-vs-layer-3) for details on what's available for remote subnets.

For best results, deploy a daemon on each network segment. See [Multi-VLAN Deployment](/multi-vlan/).
