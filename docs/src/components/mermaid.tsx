...
export function Mermaid({ chart, enableZoom = true }: MermaidProps) {
	...
	if (error) {
		return (
			<div className="mermaid-error my-4 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
				<p className="text-sm text-red-400">Failed to render diagram: {error}</p>
				<pre className="mt-2 overflow-auto text-xs text-gray-400">{chart}</pre>
			</div>
		);
	}
	...
	return (
		...
		>
			...
			>
				...
				>
					<div className="mermaid-diagram-inner" dangerouslySetInnerHTML={{ __html: svg }} />
				</TransformComponent>
			</TransformWrapper>
		</div>
	);
