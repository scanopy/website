<script lang="ts">
	...
	const canonical = 'https://scanopy.net/comparisons/scanopy-alternatives';
	...
	// Schema: each alternative as a SoftwareApplication inside an ItemList, plus a
	// BreadcrumbList and the FAQPage.
	...
	const headings = $derived([
		{ id: 'at-a-glance', text: 'At a glance', level: 2 },
		{ id: 'closest-alternatives', text: 'Closest alternatives', level: 2 },
		...
		<div class:blog-layout={showToc}>
			<div class="blog-content">
				...
				<div class="prose prose-invert prose-gray max-w-none">
					...
					<h2 id="at-a-glance">Scanopy alternatives at a glance</h2>
					<p>
						How Scanopy and the closest alternatives compare on discovery, the four topology views
					...
					<h2 id="closest-alternatives">The closest Scanopy alternatives</h2>
				</div>
				...
				<div class="my-8 space-y-4">
					{#each data.alternatives as alt (alt.slug)}
						<div class="rounded-lg border border-gray-800 p-5">
							...
							<p class="mb-0 mt-2 text-sm leading-relaxed text-gray-400">
								<span class="font-semibold text-gray-300">Where Scanopy differs:</span>
								{alt.edge}
							</p>
							<a
								href={alt.vsHref}
...
<style>
	...
	/* Shared comparison-table + chip styles, mirrored from the alternatives/[slug] page so
	   the VendorComparison inline table's view-type tags and chips render identically here. */
	:global(.prose h2) {
		margin-top: 3rem;
		margin-bottom: 1rem;
		font-size: 1.5rem;
