...
- Add mDNS/DNS-SD browsing to network discovery, so AirPlay speakers, printers, Chromecasts, and other advertised services show up automatically — announcements also count as proof a host is alive, and matching uses the full service record rather than just the service type.
  ...
  ![Discovery results showing hosts found via mDNS/DNS-SD with their advertised services](/changelog/v0.17.13/mdns-discovery.png)
...
- Show the machine running the Scanopy daemon as a host in its own right, complete with its own interfaces and addresses.

  ![Host detail view for the daemon machine listing its network interfaces](/changelog/v0.17.13/daemon-host.png)
...
- Stop the ping sweep from probing network and broadcast addresses.
