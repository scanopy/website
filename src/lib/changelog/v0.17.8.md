...
This release focuses on telling you *why* a scan came back thin — unused or unreachable credentials, missing SNMP data, and blank topology canvases now explain themselves instead of leaving you guessing.
...
- Add an aggregated scan warnings summary that lists every credential Scanopy tried, flags the ones that were unreachable or never used, and says what kind of failure each attempt hit ([#667](https://github.com/scanopy/scanopy/pull/667)).
  ...
  ![Scan results panel showing a list of scan warnings including unused and unreachable credentials](/changelog/v0.17.8/scan-warnings.png)
...
- Surface a clear message when the topology canvas renders empty, so a blank map is distinguishable from a still-loading one ([#667](https://github.com/scanopy/scanopy/pull/667)).
- Publish the daemon provisioning, topology, and user-update endpoints in the API documentation, and close the remaining gaps in the generated OpenAPI spec ([#667](https://github.com/scanopy/scanopy/pull/667)).
...
- Speed up page loads by only fetching Stripe's billing script when you actually open billing ([#667](https://github.com/scanopy/scanopy/pull/667)).
...
- Fix a blank topology canvas caused by the graph opening at a collapse level that didn't match what was actually drawn ([#667](https://github.com/scanopy/scanopy/pull/667)).
