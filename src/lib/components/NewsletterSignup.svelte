	...
	<form onsubmit={handleSubmit} class="space-y-3 {className}">
		...
		<div class="flex gap-2 {compact ? 'flex-col sm:flex-row' : ''}">
			<input
				...
				oninput={reset}
				disabled={loading}
				class="min-w-0 flex-1 rounded-lg border border-gray-700 bg-gray-800/50 px-4 py-2 text-sm text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50 {status ===
				'error'
					? 'border-red-500'
					: ''}"
			/>
